...
module.exports = function (models) {
    return new function () {
        ...
        this.createMulti = function (options, callback) {
            ...
            async.eachSeries(options.availabilities, function(item, cb1){
                Availability.findOne({location:item.location, product:item.product}, function (err, result) {
                    if (err) {
                        return next(err);
                    }
                    if(!result){
                        var availability;
                        item.goodsInNotes = [{goodsNoteId : item.goodsInNote, quantity : item.onHand}];
                        delete item.goodsInNote;
                        availability = new Availability(item);
                        availability.save(function (err, doc) {
                            if (err) {
                                return cb1(err);
                            }
                        ...
                        result.goodsInNotes.push({goodsNoteId : item.goodsInNote, quantity : item.onHand});
                        result.onHand+=item.onHand;
                        Availability.findByIdAndUpdate(result._id, {goodsInNotes: result.goodsInNotes, onHand: result.onHand, cost: item.cost}, {new: true}, function (err, result1) {
                            if (err) {
                                return cb1(err);
                            }
                            cb1(null, result1);
