...
var Module = function (models, event) {
    ...
    var projectRoyalty=require('./projectRoyalty');
    var projectRoyaltyHandler = new projectRoyalty(models);
    ...
    this.createManual = function (req, res, next) {
        ...
        function journalEntrySave(journal, waterfallCb) {
            ...
            currency = {
                _id : currency._id,
                rate: 1 //ratesRetriever.getRate(fx.rates, fx.base, currency.name)
            };
...
/**
        ...
        function projectRoyaltyUpdate(waterfallCb) {
            projectRoyaltyHandler.calculateRoyalty(req, res, next,accountArray)
        }

        waterfallTasks = [ journalFinder, journalEntrySave,projectRoyaltyUpdate];  // remove getRates
    ...
    this.getSourceForDd = function (req, res, next) {
        ...
        var Job = models.get(dbName, 'jobs', jobsSchema);
        var Payment = models.get(dbName, 'InvoicePayment', InvoicePaymentSchema);
        var BuildingModel = models.get(dbName, 'building', buildingSchema);
        var parallelTasks;
        ...
        function getBuilding(pCB) {
            BuildingModel.aggregate([
                {
                    $project: {
                        _id : 1,
                        name: 1
        ...
        parallelTasks = [getEmployees, getJobs, getInvoice, getPayments,getBuilding];
        //parallelTasks = [getBuilding];
