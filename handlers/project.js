module.exports = function (models, event) {
    ...
    var lookupForProjectArray = [
        ...
        },
        {
            ...
            $project: {
                ...
                pmrname:{$concat: ['$pmr.name.first', ' ', '$pmr.name.last']},
    ...
    this.getByViewType = function (req, res, next) {
        ...
        if (viewType === 'list') {
            ...
            projectionOptions.pmr = 1;
            projectionOptions.pmv = 1;
            ...
            projectionLastStepOptions.pmr = '$root.pmr';
            projectionLastStepOptions.pmv = '$root.pmv';
    ...
    this.getById = function (req, res, next) {
        ...
        Project.aggregate([{
            ...
            $project: {
                ...
                pmr: {
                    _id     : '$pmr._id',
                    fullName: {$concat: ['$pmr.name.first', ' ', '$pmr.name.last']}
                },
                pmv: {
                    _id     : '$pmv._id',
                    fullName: {$concat: ['$pmv.name.first', ' ', '$pmv.name.last']}
                },
                archiveMan:{
                    _id     :'$archiveMan._id',
    ...
    this.getForInvoice=function(req,res,next){
        var invoice = models.get(req.session.lastDb, 'Invoice', InvoiceSchema);
        var data = req.query;
        var id = data ? data._id : null;
        var projectSearch;
        ...
        invoiceSearch =function(waterfallCallback){
        ...
        waterfallTasks = [invoiceSearch];
        async.waterfall(waterfallTasks, function (err, result) {
            if (err) {
                return next(err);
            }
            projectInvoice.project=project;
            projectInvoice.invoice=result;
            res.status(200).send(projectInvoice);
        });
    };
