...
var tasksSchema = mongoose.Schemas.internalContract;
...
var Module = function (models, event) {
    ...
    this.createTask = function (req, res, next) {
        ...
        var TasksModel = models.get(req.session.lastDb, 'internalContract', tasksSchema);

        body.uId = req.session.uId;

        TasksModel.find({_type:'internalContract'})
            .exec(function (err,taskNumber) {
                var n;
                var task;
                n=taskNumber.length;
                body.number='IC_'+(n*1+1);
                body.createdBy = {
                    date: new Date(),
    ...
    this.uploadFile = function (req, res, next) {
        var Model = models.get(req.session.lastDb, 'internalContract', tasksSchema);
    ...
    this.taskUpdateOnlySelectedFields = function (req, res, next) {
        ...
        function updateTask() {
            models.get(req.session.lastDb, 'internalContract', tasksSchema).findByIdAndUpdate(_id, {$set: data}, {new: true}, function (err, result) {
        ...
        function sequenceUpdate() {
            if (data.sequence === -1) {
                event.emit('updateSequence', models.get(req.session.lastDb, 'internalContract', tasksSchema), 'sequence', data.sequenceStart, data.sequence, data.workflowStart, data.workflowStart, false, true, function () {
                    event.emit('updateSequence', models.get(req.session.lastDb, 'internalContract', tasksSchema), 'sequence', data.sequenceStart, data.sequence, data.workflow, data.workflow, true, false, function (sequence) {
                ...
                event.emit('updateSequence', models.get(req.session.lastDb, 'internalContract', tasksSchema), 'sequence', data.sequenceStart, data.sequence, data.workflowStart, data.workflow, false, false, function (sequence) {
    ...
    function getTaskById(req, res, next) {
        var data = req.query;
        var Tasks = models.get(req.session.lastDb, 'internalContract', tasksSchema);
    ...
    function getTasksForList(req, res, next) {
        ...
        var obj = {};
        var addObj = {};
        var Task = models.get(req.session.lastDb, 'internalContract', tasksSchema);
    ...
    this.bulkRemove = function (req, res, next) {
        ...
        async.each(ids, function (id, cb) {
            Model.findByIdAndRemove(id, function (err, task) {
                ...
                event.emit('updateSequence', models.get(req.session.lastDb, 'internalContract', tasksSchema), 'sequence', task.sequence, 0, task.workflow, task.workflow, false, true);
    ...
    this.removeTask = function (req, res, next) {
        ...
        models.get(req.session.lastDb, 'internalContract', tasksSchema).findByIdAndRemove(_id, function (err, task) {
            if (err) {
                return next(err);
            }
