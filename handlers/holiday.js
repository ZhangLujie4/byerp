...
var Module = function (models, event) {
    ...
    this.bulkRemove = function (req, res, next) {
        ...
        async.each(ids, function (id, cb) {
            Holiday.findByIdAndRemove(id, function (err, holiday) {
                ...
                updateTimeCard(req, holiday, false, function(err, result){
                    if(err){
                        cb(err);
                    }
                    event.emit('setReconcileTimeCard', {req: req, week: holiday.week, year: holiday.year});
    ...
    this.create = function (req, res, next) {
        ...
        Holiday.save(function (err, holiday) {
            ...
            updateTimeCard(req, body, true, function(err){
                if(err){
                    next(err);
                }
                res.status(200).send({success: holiday});
    ...
    function updateTimeCard(req, body, isCreate, cb) {
        var timeCard = models.get(req.session.lastDb, 'timeCard', tcardSchema);
        var date = moment(body.date);
        var type = body.type;
        var year = date.year();
            ...
            if(!result.length){
                ...
                if(isCreate){
                    async.each(result, function(item, asyncCb){
                        timeCard.findByIdAndUpdate(item._id, {rate: item.rate*10}, function(err, result){
                            if(err){
                                asyncCb(err)
                            }
                            asyncCb();
