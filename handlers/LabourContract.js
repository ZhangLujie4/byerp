...
var LabourContractSchema = mongoose.Schemas.labourContract;
...
var Module = function (models, event) {
    ...
    this.createTask = function (req, res, next) {
        ...
        var TasksModel = models.get(req.session.lastDb, 'labourContract', LabourContractSchema);

        body.uId = req.session.uId;

        TasksModel.find({_type:'labourContract'})
            .sort({count: -1})
    ...
    this.uploadFile = function (req, res, next) {
        var Model = models.get(req.session.lastDb, 'labourContract', LabourContractSchema);
    ...
    this.taskUpdateOnlySelectedFields = function (req, res, next) {
        ...
        function updateTask() {
            models.get(req.session.lastDb, 'labourContract', LabourContractSchema).findByIdAndUpdate(_id, {$set: data}, {new: true}, function (err, result) {
        ...
        function sequenceUpdate() {
            if (data.sequence === -1) {
                event.emit('updateSequence', models.get(req.session.lastDb, 'labourContract', LabourContractSchema), 'sequence', data.sequenceStart, data.sequence, data.workflowStart, data.workflowStart, false, true, function () {
                    event.emit('updateSequence', models.get(req.session.lastDb, 'labourContract', LabourContractSchema), 'sequence', data.sequenceStart, data.sequence, data.workflow, data.workflow, true, false, function (sequence) {
                ...
                event.emit('updateSequence', models.get(req.session.lastDb, 'labourContract', LabourContractSchema), 'sequence', data.sequenceStart, data.sequence, data.workflowStart, data.workflow, false, false, function (sequence) {
            ...
            function (err, deps) {
                ...
                models.get(req.session.lastDb, 'Project', projectSchema).aggregate(
                    ...
                    function (err, projectsId) {
                        ...
                        query = models.get(req.session.lastDb, 'Tasks', LabourContractSchema).where('project').in(projectsId.objectID()).where('workflow', objectId(data.workflowId));
                        ...
                        query.select('_id assignedTo  pmr customer workflow editedBy.date archedBy.date project taskCount summary type remaining priority sequence')
                            ...
                            .exec(function (err, result) {
                                ...
                                responseData.fold = (req.session.kanbanSettings.LabourContract.foldWorkflows && req.session.kanbanSettings.LabourContract.foldWorkflows.indexOf(data.workflowId.toString()) !== -1);
    ...
    function getTaskById(req, res, next) {
        var data = req.query;
        var Tasks = models.get(req.session.lastDb, 'labourContract', LabourContractSchema);
    ...
    function getTasksForList(req, res, next) {
        ...
        var obj = {};
        var addObj = {};
        var Task = models.get(req.session.lastDb, 'labourContract', LabourContractSchema);
    ...
    this.bulkRemove = function (req, res, next) {
        var Model = models.get(req.session.lastDb, 'Tasks', LabourContractSchema);
        ...
        async.each(ids, function (id, cb) {
            Model.findByIdAndRemove(id, function (err, task) {
                ...
                event.emit('updateSequence', models.get(req.session.lastDb, 'Tasks', LabourContractSchema), 'sequence', task.sequence, 0, task.workflow, task.workflow, false, true);
    ...
    this.removeTask = function (req, res, next) {
        ...
        models.get(req.session.lastDb, 'labourContract', LabourContractSchema).findByIdAndRemove(_id, function (err, task) {
            if (err) {
                return next(err);
            }

            event.emit('updateContent', req, res, task.project, 'remove');
            event.emit('updateSequence', models.get(req.session.lastDb, 'labourContract', LabourContractSchema), 'sequence', task.sequence, 0, task.workflow, task.workflow, false, true);
    ...
    this.getLengthByWorkflows = function (req, res, next) {
        ...
        var Tasks = models.get(req.session.lastDb, 'Tasks', LabourContractSchema);
        var data = {};
        var addObj = {};
