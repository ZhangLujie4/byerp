...
var Employee = function (event, models) {
                    ...
                    switch (viewType) {
                        case ('list'):
                            ...
                            projectSecond = {
                                ...
                                'department._id'  : '$department._id',
                                'department.name' : '$department.name',
                                'department.nestingLevel' : '$department.nestingLevel',
                                'createdBy.user'  : 1,
                                'editedBy.user'   : 1,
                                'editedBy.date'   : 1,
                                'createdBy.date'  : 1,
                    ...
                    switch (viewType) {
                        case ('list'):
                            ...
                            projectAfterRoot = {
                                ...
                                'department._id'  : '$root.department._id',
                                'department.name' : '$root.department.name',
                                'department.nestingLevel' : '$root.department.nestingLevel',
                                'jobType'         : '$root.jobType',
                                name              : '$root.name',
                                dateBirth         : '$root.dateBirth',
                                skype             : '$root.skype',
    ...
    this.create = function (req, res, next) {
        var dbName = req.session.lastDb;
        var Model = models.get(dbName, 'Employees', EmployeeSchema);
        var departmentModel = models.get(dbName,'Department',DepartmentSchema);
    ...
    this.updateOnlySelectedFields = function (req, res, next) {
        ...
        console.log('data.sequence',data.sequenceStart );
        console.log('data.workflowStart',data.workflowStart);
    ...
    this.forProfile = function (req, res, next) {
        ...
        function findEmployee(user, waterfallCb) {
            ...
            getById(req, res, function (err, employee) {
                if (err) {
                    return waterfallCb(err);
                }
   ...
   this.getBirthdays = function (req, res, next) {
    ...
    //app
    this.appLogin = function(req,res,next){
        var retObj = {};
        var deviceid = req.body.deviceid;
        var workid = req.body.workid;
        var userpasswd = req.body.password;
        var dbName = req.session.lastDb || 'CRM';
        console.log("req.body",req.body);
        var employeeModel = models.get(dbName, 'Employees', EmployeeSchema);
        employeeModel.find({workId: workid})
        .select('deviceId workId jobPosition name passWord workPhones workCentre')
        .populate('workCentre', '_id name')
        .exec(function (err, employeeInfo) {
            ...
            if(!employeeInfo[0]){
                ...
                retObj.retError='员工不存在';
                res.status(200).send(retObj);
            }
            if(employeeInfo[0].deviceId == ''){
                console.log("employeeInfo[0].deviceId",employeeInfo[0].deviceId);
                if(userpasswd == employeeInfo[0].passWord){
                    employeeModel.findByIdAndUpdate(employeeInfo[0]._id, {$set:{deviceId:deviceid}},{new:true},function (error, result) {
                        if (error) {
                            return next(error);   
                        }
                        ...
                        retObj.retValue = {
                            ...
                            station:employeeInfo[0].workCentre.name,
                            jobPosition:employeeInfo[0].workCentre._id,
                            workPhones:result.workPhones,
                            _id:employeeInfo[0]._id,
                          password:employeeInfo[0].passWord
                        };
                        console.log("diINFO",retObj);
                        res.status(200).send(retObj);
                    });
                }else{
                    retObj.retStatus='Fail';
                    retObj.retError='密码错误';
                    res.status(200).send(retObj);
                }

            }else /*if(deviceid == employeeInfo[0].deviceid){
                if(userpasswd != employeeInfo[0].passWord){
                   ...
                   if(userpasswd != employeeInfo[0].passWord){
                     ...
                     employeeModel.findById(employeeInfo[0]._id, function (error, result) {
                      if (error) {
                          return next(error);   
                      }
    ...
    this.getForDdByDeptId = function (req, res, next) {
        var Model = models.get(req.session.lastDb, 'Employees', EmployeeSchema);
        var result = {};
        var deptId = req.params.id;

        var query = Model.find({department: deptId, isEmployee: true}, {name: 1}).sort({'name.first': 1});
    ...
    this.changePass = function (req, res, next){
        ...
        var prePass = body.prePass;
        var newPass = body.newPass;
        var retObj = {};
        console.log(body);
        Model.findById(employee, function(err, result){
            if(err){
                next(err);
            }
            var passWord = result.passWord;
            if(prePass.toString() == passWord.toString()){
                Model.findByIdAndUpdate(employee, {passWord: newPass}, function(err, result){
                    if(err){
                        next(err)
                    }
                    retObj.retStatus='OK';
                    retObj.retValue='修改密码成功';
                    res.status(200).send(retObj);
                })
            }
            else{
                retObj.retStatus='Fail';
                retObj.retError='原密码错误，请重新输入';
                res.status(200).send(retObj);
            }
        })
    };
