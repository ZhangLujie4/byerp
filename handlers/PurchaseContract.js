...
var Module = function (models, event) {
    ...
    this.createTask = function (req, res, next) {
        ...
        var TasksModel = models.get(req.session.lastDb, 'purchaseContract', QuotationContractSchema);

        body.uId = req.session.uId;


        TasksModel.find({_type:'purchaseContract'})
            .sort({count: -1})
    ...
    this.uploadFile = function (req, res, next) {
        var Model = models.get(req.session.lastDb, 'purchaseContract', QuotationContractSchema);
    ...
    this.taskUpdateOnlySelectedFields = function (req, res, next) {
        ...
        function updateTask() {
            models.get(req.session.lastDb, 'purchaseContract', QuotationContractSchema).findByIdAndUpdate(_id, {$set: data}, {new: true}, function (err, result) {
        ...
        function sequenceUpdate() {
            if (data.sequence === -1) {
                event.emit('updateSequence', models.get(req.session.lastDb, 'purchaseContract', QuotationContractSchema), 'sequence', data.sequenceStart, data.sequence, data.workflowStart, data.workflowStart, false, true, function () {
                    event.emit('updateSequence', models.get(req.session.lastDb, 'purchaseContract', QuotationContractSchema), 'sequence', data.sequenceStart, data.sequence, data.workflow, data.workflow, true, false, function (sequence) {
                ...
                event.emit('updateSequence', models.get(req.session.lastDb, 'purchaseContract', QuotationContractSchema), 'sequence', data.sequenceStart, data.sequence, data.workflowStart, data.workflow, false, false, function (sequence) {
            ...
            function (err, deps) {
                ...
                models.get(req.session.lastDb, 'Project', projectSchema).aggregate(
                    ...
                    function (err, projectsId) {
                        ...
                        query = models.get(req.session.lastDb, 'Tasks', QuotationContractSchema).where('project').in(projectsId.objectID()).where('workflow', objectId(data.workflowId));
                        ...
                        query.select('_id assignedTo  pmr customer workflow editedBy.date archedBy.date project taskCount summary type remaining priority sequence')
                            ...
                            .sort({sequence: -1})
                            .limit(req.session.kanbanSettings.QuotationContract.countPerPage)
                            .exec(function (err, result) {
                                ...
                                responseData.fold = (req.session.kanbanSettings.QuotationContract.foldWorkflows && req.session.kanbanSettings.QuotationContract.foldWorkflows.indexOf(data.workflowId.toString()) !== -1);
    ...
    function getTaskById(req, res, next) {
        ...
        Tasks.findById(data.id)
            ...
            .populate('products.product', '_id name info variants')
            .populate('products.sourceAl', '_id name')
            ...
            .exec(function (err, task) {
                ...
                var pro=task.products;
                var productArray=[];
                async.each(pro,function (products,cb1) {
                    var productItem={};
                    productItem.qty1=products.qty1;
                    productItem.qty2=products.qty2;
                    productItem.qty3=products.qty3;
                    productItem.qty4=products.qty4;
                    productItem.qty5=products.qty5;
                    productItem.price1=products.price1;
                    productItem.price2=products.price2;
                    productItem.price3=products.price3;
                    productItem.price4=products.price4;
                    productItem.price5=products.price5;
                    productItem.sourceAl=products.sourceAl;
                    productItem.quantity=products.quantity;
                    productItem.unitPrice=products.unitPrice;
                    productItem.subTotal=products.subTotal;
                    Product
                        .aggregate([
                            {
                              $match:{_id:objectId(products.product._id)}
                            },
                            {
                                $project:{
                                    name:1,
                            ...
                            productItem.product=result[0];
                            productArray.push(productItem);
                            if (err) {
                                return cb1(err);
                            }
                            cb1(null, productArray);
                        });
                },function(err){
                    if(err){
                        return next(err);
    ...
    function getTasksForList(req, res, next) {
        ...
        var obj = {};
        var addObj = {};
        var Task = models.get(req.session.lastDb, 'purchaseContract', QuotationContractSchema);
    ...
    this.bulkRemove = function (req, res, next) {
        var Model = models.get(req.session.lastDb, 'Tasks', QuotationContractSchema);
        ...
        async.each(ids, function (id, cb) {
            Model.findByIdAndRemove(id, function (err, task) {
                ...
                event.emit('updateSequence', models.get(req.session.lastDb, 'Tasks', QuotationContractSchema), 'sequence', task.sequence, 0, task.workflow, task.workflow, false, true);
    ...
    this.removeTask = function (req, res, next) {
        ...
        models.get(req.session.lastDb, 'purchaseContract', QuotationContractSchema).findByIdAndRemove(_id, function (err, task) {
            if (err) {
                return next(err);
            }

            event.emit('updateContent', req, res, task.project, 'remove');
            event.emit('updateSequence', models.get(req.session.lastDb, 'purchaseContract', QuotationContractSchema), 'sequence', task.sequence, 0, task.workflow, task.workflow, false, true);
    ...
    this.getLengthByWorkflows = function (req, res, next) {
        ...
        var Tasks = models.get(req.session.lastDb, 'Tasks', QuotationContractSchema);
        var data = {};
        var addObj = {};
