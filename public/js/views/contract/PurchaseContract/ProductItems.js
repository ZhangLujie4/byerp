define([
    ...
    'jQuery',
    'Underscore',
    'text!templates/contract/PurchaseContract/product/ProductItems.html',
    'text!templates/contract/PurchaseContract/product/ProductInputContent.html',
    'text!templates/contract/PurchaseContract/product/ProductItemsEditList.html',
    'text!templates/contract/PurchaseContract/product/ItemsEditList.html',
    'text!templates/contract/PurchaseContract/product/TotalAmount.html',
    ...
    var ProductItemTemplate = Backbone.View.extend({
        ...
        events: {
            ...
            'keyup td[data-name=price],td[data-name=processingCharges],td[data-name=quantity] input'                  : 'priceChange',
            ...
            'keyup td[data-name=subtotal] input'                                      : 'priceChange'
        },
        ...
        getProducts: function (e) {
            ...
            if (1/*rowId === undefined ||  rowId !== 'false' !hasError*/) {
                if (!$trEll.length) {
        ...
        chooseOption: function (e) {
            ...
            var type=$parrent.attr('data-name');
            ...
            var $qty1 = $trEl.find('.qty1');
            var $qty2 = $trEl.find('.qty2');
            var $qty3 = $trEl.find('.qty3');
            var $qty4 = $trEl.find('.qty4');
            var $qty5 = $trEl.find('.qty5');

            //var sourceAl=document.getElementById('sourceAl');
            var sourceAl=$target.parents('tr').find("[data-name='sourceAl'] #sourceAl");
            ...
            if(type!='sourceAl') {
                ...
                var parameter = selectedProduct.parameter;
                var productType = selectedProduct.productType.name;


                if (!isJob) {
                    if(selectedProduct.variants){
                        var str='';
                        for(var m=0;m<selectedProduct.variants.length;m++){
                            str=str+selectedProduct.variants[m].optionId.name+selectedProduct.variants[m].value+';';
                        }
                    }
                    description = selectedProduct && selectedProduct.variants ? str : '';
                    $descriptionContainer.val(description);
                    for (var a = 0; a < parameter.length; a++) {
                        if (parameter[a].seq == 1) {
                            qty1 = parameter[a].name;
                        }
                        if (parameter[a].seq == 2) {
                            qty2 = parameter[a].name;
                        }
                        if (parameter[a].seq == 3) {
                            qty3 = parameter[a].name;
                        }
                        if (parameter[a].seq == 4) {
                            qty4 = parameter[a].name;
                        }
                        if (parameter[a].seq == 5) {
                            qty5 = parameter[a].name;
                        }
                    }
                    $qty1.val(qty1);
                    $qty2.val(qty2);
                    ...
                    var AL=productType.indexOf('铝');

                    if (AL>=0) {
                        var ifhave=$trEl.find('#alumSource').text();
                        if(!ifhave) {
                            //sourceAl.innerHTML = "<table class='list stripedList'> <colgroup> <col width='100%'> </colgroup> <thead> <tr> <th> 铝锭价来源</th> </tr> </thead> <tbody> <td> <input id='sourceAl' class='sourceAl' maxlength='9' size=7 type='text' /></td></tbody> </table>";
                            sourceAl.append("   <dd class='_newSelectListWrap' style='100px'> <a id='alumSource' class='current-selected' href='javascript:;'>铝锭价来源</a> </dd>  ");
                        }
                    }
        ...
        processingChargesRetriver: function ($parent) {
            var selectedProduct = this.products || new Backbone.Collection();
            var id;
            var processingCharges;
            ...
            if (selectedProduct && selectedProduct.get('name') === CONSTANTS.IT_SERVICES) {
                ...
                processingCharges = $parent.find('#processingCharges').val() || $parent.find('td[data-name="processingCharges"]').text();
                //quantity = $.trim($parent.find('[data-name="quantity"]').text());
                processingCharges = parseFloat(processingCharges);
            }

            processingCharges = this.isNaN(processingCharges);
        ...
        recalculateTaxes: function ($parent) {
            ...
            //processingCharges = $parent.find('[data-name="processingCharges"] input').val() || $parent.find('[data-name="processingCharges"]').text();
            cost = parseFloat(helpers.spaceReplacer(cost)) || 0;
            //processingCharges=parseFloat(helpers.spaceReplacer(processingCharges)) || 0;

            //total = quantity * (cost+processingCharges);
            total = quantity * cost;
            taxes = total * this.taxesRate;
            subtotal = total + taxes;
           ...
           // var processingCharges;
            ...
            if (totalEls) {
                for (i = totalEls - 1; i >= 0; i--) {
                   ...
                   // processingCharges = $currentEl.find('[data-name="processingCharges"] input').val() || $currentEl.find('[data-name="processingCharges"]').text() || '0';
                    quantity = this.quantityRetriver($currentEl);
                    subtotal=$currentEl.find('[data-name="subtotal"] input').val() || $currentEl.find('[data-name="subtotal"]').text()|| $currentEl.find('.subtotal').text()||0;


                   cost = helpers.spaceReplacer(cost);
                   cost = parseFloat(cost) || 0;
                   subtotal=helpers.spaceReplacer(subtotal);
                   //processingCharges = helpers.spaceReplacer(processingCharges);
                   // processingCharges = parseFloat(processingCharges) || 0;
                    cost = quantity *cost;
                    totalUntax += subtotal*1;
            ...
            if (options && options.model) {
                ...
                if (products) {
                    ...
                    for(i=length-1;i>=0;i--) {
                        ...
                        unitPrice = products[i].unitPrice || 0;
                        subTotal = products[i].subTotal || 0;
                    ...
                    total = (total).toFixed(decimalPlace);
                    options.model.productsTotal=total;
