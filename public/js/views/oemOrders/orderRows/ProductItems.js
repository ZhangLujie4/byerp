define([
    ...
    var ProductItemTemplate = Backbone.View.extend({
        ...
        events: {
            ...
            'keyup td[data-name=price2]'                                                   : 'recalculateDiscount'
        },
        ...
        recalculateDiscount: function (e) {
            var $target = this.$el.find('#discount');
        ...
        calculateTotal: function (discount) {
            ...
            if (totalEls) {
                for (i = totalEls - 1; i >= 0; i--) {
                    ...
                    vicePrice = $currentEl.find('[data-name="price2"] input').val() || '0';
                    vicepriceQty = $currentEl.find('[data-name="priceQty2"] input').val() || '0';
                    cost = helpers.spaceReplacer(cost);
                    cost = parseFloat(cost) || 0;
                    if(priceQty != 0) {
                        cost = cost * priceQty + (vicePrice * vicepriceQty * quantity/1000);
                    }else{
                        cost = 0;
                    }
                    subtotal = cost.toFixed(2);
                    $currentEl.find('.subtotal .sum').text(helpers.currencySplitter(subtotal));
