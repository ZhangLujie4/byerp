define([
    ...
    var CreateView = ParentView.extend({
        ...
        saveItem: function () {
            ...
            var isComproject = $.trim($thisEl.find('#isComproject').val());
            var regDepart = $thisEl.find('#regDepart').attr('data-id');
            var proManager = $.trim($thisEl.find('#proManager').val());
            var transactor = $.trim($thisEl.find('#transactor').val());

            var recordDate = $.trim($thisEl.find('#recordDate').val());
            var bidDepartment = $.trim($thisEl.find('#bidDepartment').val());
            var isBid = $.trim($thisEl.find('#isBid').val());
            ...
            this.model.save({
                ...
                reffered        : reffered,
                workflow        : workflow,
                isComproject    : isComproject,
                regDepart       : regDepart,
                proManager      : proManager,
                transactor      : transactor,
                recordDate      : recordDate,
                bidDepartment   : bidDepartment,
                isBid           : isBid,
        ...
        render: function () {
            ...
            populate.get('#regDepart', '/departments', {}, 'name', this, true, true);
            ...
            this.$el.find('#recordDate').datepicker({
                dateFormat: 'yy-MM-dd',
                monthNames: ['01','02','03','04','05','06', '07','08','09','10','11','12'],
                monthNamesShort: ['一月','二月','三月','四月','五月','六月', '七月','八月','九月','十月','十一月','十二月'],
                dayNamesMin: ['日','一','二','三','四','五','六'], 
