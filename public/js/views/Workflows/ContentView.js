define([
    ...
    var ContentView = Backbone.View.extend({
            ...
            model.save({}, {
                ...
                success: function () {
                    ...
                    $targetParent.siblings().find('span').removeClass('hidden').end()
                        .find('input, select, a:contains("取消"), a:contains("保存")').remove();
                    $targetParent.find('.edit').removeClass('hidden').end().find('a:contains("取消"), a:contains("取消")').remove();
                    $targetParent.find('.delete').removeClass('hidden').end().find('a:contains("保存"), a:contains("保存")').remove();
                    $thisEl.find('#addNewStatus').show();
                },
        ...
        cancel: function (e) {
            ...
            $targetParent.siblings().find('span').removeClass('hidden').end()
                .find('input, select, a:contains("取消"), a:contains("保存")').remove();
            $targetParent.find('.edit').removeClass('hidden').end().find('a:contains("取消"), a:contains("保存")').remove();
            $targetParent.find('.delete').removeClass('hidden');
            $thisEl.find('#addNewStatus').show();
        },
        ...
        edit: function (e) {
            ...
            $thisEl.find('.name input, input.nameStatus, select, a:contains("取消"), a:contains("保存")').remove();
            ...
            $td.append(
                $(text).text('保存').addClass('save'),
                $(text).text('取消').addClass('cancel')
            );
        },
       ...
       /* createItem: function () {
        ...
        addNewStatus: function (e) {
            ...
            e.preventDefault();
            $thisEl.find('span').removeClass('hidden');
            $thisEl.find('.name input, select, a:contains("取消"), a:contains("保存")').remove();
            $thisEl.find('.edit').removeClass('hidden');
            $thisEl.find('.delete').removeClass('hidden');
            ...
            $thisEl.find('#workflows').append('<div class="addnew row"><div><input type="text" class="nameStatus" maxlength="32" required /></div>' +
                ...
                '<div class="SaveCancel"><a href="javascript:;" id="saveStatus">保存</a>' +
                '<a  href="javascript:;" id="cancelStatus">取消</a></div></div>');
