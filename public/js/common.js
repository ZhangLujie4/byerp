define([
    ...
    var checkBackboneFragment = function (url) {
        ...
        //Backbone.History 负责整个页面的历史记录和导航行为管理，因此在整个生命周期内应该保持唯一
        //start() 方法触发后，将创建 Backbone.history 的 fragment 属性。它包含散列片段的值
    ...
    var utcDateToLocaleDate = function (utcDateString, hours) {
        ...
        if (hours) {
            utcDateString = utcDateString ? moment(utcDateString).format("YYYY-MM-DD,  HH:mm A") : null;
        } else {
            utcDateString = utcDateString ? moment(utcDateString).format("YYYY-MM-DD ") : null;
        }
    ...
    var utcDateToLocaleFullDateTime = function (utcDateString) {
        ...
        return moment(utcDateString).format('dddd, YYYY-MM-DD HH:mm A');
    };
    ...
    var populateProjectsDd = function (selectId, url, model, callback) {
        ...
        selectList.append($("<option/>").val('').text('选择...'));
    ...
    var getGoodsInNotes = function (filter, forSales, callback) {
        dataService.getData('/goodsInNotes/getGoodsInNote', {filter: filter, forSales: forSales},
            function (response) {
                callback(response);
            });
    };
    ...
    return {
        ...
        getGoodsInNotes                   : getGoodsInNotes
    }
});
